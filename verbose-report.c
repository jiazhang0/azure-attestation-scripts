// https://github.com/Azure-Samples/aci-confidential-hello-world/blob/main/app/verbose-report.c
...
int main(int argc, char** argv)
{
    ...
    if (argc > 1 && !strcmp(argv[1], "--help")) {
        printf("%s: [--help|--html|--raw|--example|--out]\n", argv[0]);
        exit(0);
    }
...
// use --example to get a canned report, used to help debug exposing reports via HTTP etc 
    ...
    if (saveOut) {
        ...
        fp = fopen("report.bin", "wb");
        if (!fp) {
            printf("Failed to save the report\n");
            exit(-1);
        }

        fwrite(&msg_report_out.report, sizeof(msg_report_out.report), 1, fp);
